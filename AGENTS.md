...
- `.claude-plugin/marketplace.json` — Anthropic plugin format. The **pricing** plugin lists the two pricing skills; the `narev` router skill lives alongside them under `skills/` for agents that load the tree directly.
...
2. Put the router in `skills/narev/`; pricing-related skills under `skills/pricing/`.
